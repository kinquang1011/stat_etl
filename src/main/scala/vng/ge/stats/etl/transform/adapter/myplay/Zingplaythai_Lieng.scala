...
/**
  ...
  */
class Zingplaythai_Lieng extends MyplayFormatter ("zingplaythai_lieng") {
  ...
  override def getPaymentDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val paymentPatternPath = Constants.GAMELOG_DIR + "/myplay_payment_db/[yyyyMMdd]/Cash_liengsea_th*"
    ...
    val paymentDs = paymentRaw.map(line => line.split("\\t")).filter(line => filterLog(line(6))).map { line =>
      ...
      val money = line(4)
      val id = line(2)
      ("zingplaythai_lieng", dateTime, id, money, money)
  ...
  override def getActivityDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val loginQuery = s"select * from zingplaythai_lieng.login where ds ='$logDate'"
      loginDsFromHive = getHiveLog(loginQuery)
      val logoutQuery = s"select * from zingplaythai_lieng.logout where ds ='$logDate'"
    ...
    ds = ds.withColumn("game_code", lit("zingplaythai_lieng"))
    ds
  }
}