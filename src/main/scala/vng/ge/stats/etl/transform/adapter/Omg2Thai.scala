...
class Omg2Thai extends FairyFormatter("omg2thai") {
  ...
  override def getActivityDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val patternUserPath = Constants.GAMELOG_DIR + "/omg2_thai/[yyyyMMdd]/datalog/Test_Data_LOG/gamelog/web_10.17.36.5/stat.log.*"
    ...
    val logDs = logRaw.map(line => line.split("\\|")).filter(line => filterLog(line)).map { line =>
      ...
      ("omg2thai", datetime, sid, id, os, rid, ip, did, level, action)
  ...
  override def getPaymentDs(logDate: String, hourly: String): DataFrame = {
    var logRaw: RDD[String] = null
    if (hourly == "") {
      val patternPayPath = Constants.GAMELOG_DIR + "/omg2_thai/[yyyyMMdd]/datalog/Test_Data_LOG/gamelog/web_10.17.36.5/stat.log.*"
    ...
    val logDs = logRaw.map(line => line.split("\\|")).filter(line => filterLog(line)).map { line =>
      ...
      ("omg2thai", datetime, sid, id, os, rid, ip, trans_id, gross_amt, net_amt, did, channel)
