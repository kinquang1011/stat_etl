...
/**
  ...
  */
class Nlvn extends DbgGameFormatter("nlvn"){
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val patternNewReg = Constants.GAMELOG_DIR + "/gslog/nlvn/[yyyy-MM-dd]/*/new_register.*.csv"
    ...
    val newRegFilter = (line:Array[String]) => {
      var rs = false
      if (line.length>=11 &&line(2).startsWith(logDate)) {
        rs = true
      }
      rs
    }
...
//  }
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      ...
      val patternPathLogin = Constants.GAMELOG_DIR + "/gslog/nlvn/[yyyy-MM-dd]/*/login.*.csv"
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line)).map { line =>
      val dateTime = line(2)
      val id = line(1)
      val serverId = line(15)
      val action = "login"
      ("nlvn", dateTime,id,serverId,action)
    }
