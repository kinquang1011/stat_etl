...
/**
  ...
  * release: 25/10/2017
  */
class Dmn360mobi extends FairyFormatter ("dmn360mobi") {
  ...
  override def getPaymentDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val pattern = Constants.GAMELOG_DIR + "/dmn360mobi/[yyyyMMdd]/loggame/logserver/*/tab_pay_[yyyyMMdd]*.log"
    ...
    val paymentDs = paymentRaw.map(line => line.split(",")).filter(line => filter(line)).map { line =>
      ...
      ("dmn360mobi", dateTime,id,roleId,roleName,sid,os,channel,gross,net)
    }
  ...
  override def getActivityDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val patternLogin = Constants.GAMELOG_DIR + "/dmn360mobi/[yyyyMMdd]/" +
        "loggame/logserver/*/tab_login_[yyyyMMdd]*.log"
      val patternLogout = Constants.GAMELOG_DIR + "/dmn360mobi/[yyyyMMdd]/" +
    ...
    val loginDs = loginRaw.map(line => line.split(",")).filter(line => filterlogin(line)).map { line =>
      ...
      ("dmn360mobi", dateTime,id,roleId,roleName,sid,level,os,channel,action)
    }
