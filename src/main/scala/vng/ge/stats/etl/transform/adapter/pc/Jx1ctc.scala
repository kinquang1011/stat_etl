...
/**
  ...
  */
class Jx1ctc extends DbgGameFormatter("jx1ctc"){
...
//    val patternNewReg = Constants.GAMELOG_DIR + "/gslog/jx1ctc/[yyyy-MM-dd]/new_register.*.csv"
...
//  }
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      val patternPathLogin = Constants.GAMELOG_DIR + "/gslog/jx1ctc/[yyyy-MM-dd]/*/playing_time.*.csv"
    ...
    val filterlog = (line:Array[String]) => {
      var rs = false
      if (line(0).startsWith(logDate) && line.length>=8){
        rs = true
      }
      rs
    }
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line)).map { line =>
      ...
      val action = line(5)
      val serverId = line(8)
      ("jx1ctc", dateTime,id,serverId,action,roleId)
    }
      .toDF(sf.GAME_CODE, sf.LOG_DATE, sf.ID,sf.SID,sf.ACTION,sf.RID)
...
//    import sparkSession.implicits._
//    val patternPath = Constants.GAMELOG_DIR + "/gslog/jx1ctc/[yyyy-MM-dd]/ccu.*.csv"
...
//  }
  ...
  /**
     test
    jx.testActivity("2017-01-01","2017-01-02").orderBy("log_date").coalesce(1).write.mode("overwrite").format("csv").option("header", "true").save(s"tmp/jx1-crosscheck.csv")
    jx.testRevPayment("2017-01-01","2017-01-02").orderBy("log_date").coalesce(1).write.mode("overwrite").format("csv").option("header", "true").save(s"tmp/jx1-crosscheck.csv")
    jx.testPuPayment("2017-01-01","2017-01-02").orderBy("log_date").coalesce(1).write.mode("overwrite").format("csv").option("header", "true").save(s"tmp/jx1-crosscheck.csv")
    jx.testActivity("2017-01-01","2017-01-02").orderBy("log_date").coalesce(1).write.mode("overwrite").format("csv").option("header", "true").save(s"tmp/jx1-crosscheck.csv")
    jx.testActivity("2017-01-01","2017-01-02").orderBy("log_date").coalesce(1).write.mode("overwrite").format("csv").option("header", "true").save(s"tmp/jx1-crosscheck.csv")
