...
/**
  ...
  */
class Zingplaythai_Dummy extends MyplayFormatter ("zingplaythai_dummy") {
  ...
  override def getActivityDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val loginQuery = s"select * from  zingplaythai_dummy.login where ds ='$logDate'"
      loginDsFromHive = getHiveLog(loginQuery)
      val logoutQuery = s"select * from zingplaythai_dummy.logout where ds ='$logDate'"
    ...
    ds = ds.withColumn("game_code", lit("zingplaythai_dummy"))
    ds
  }
