...
class Ygh5 extends FairyFormatter("ygh5") {
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val patternlLoginRegister = Constants.GAMELOG_DIR + "/ygh5/[yyyyMMdd]/Log/daily/*/*_tb_user_[yyyyMMdd].csv"
...
//
//    if (hourly == "") {
//      val patternlLogin = Constants.GAMELOG_DIR + "/ygh5/[yyyyMMdd]/Log/daily/log_lg_operation_log_*_[yyyyMMdd].csv"
...
//    //    }
//
//    registerDs=registerDs.select("_c3","_c0","_c2","_c6","_c28","_c27").where("_c2 != '10000' and _c6 is not null")
//
//    val action = udf { (act: String) => {
//      var result = "login"
//      if(act.equalsIgnoreCase("logoff")){
//        result="logout"
//      }
//      result
//    }
...
//
//    //    val sf = Constants.FIELD_NAME
//    loginDs=loginDs.select("_c1","_c2","_c3","_c5","_c8").where("lower(_c5) like 'login' or lower(_c5) like 'logoff'")
//
//    var join = loginDs.as('a).join(registerDs.as('b), loginDs("_c2") === registerDs("_c0") && loginDs("_c1") === registerDs("_c2"), "left_outer")
//      .select("_c1","a._c3","_c5","_c8","b._c3","_c6").where("_c0 is not null")
//
//
//    join = join.withColumn(sf.ID,col("b._c3"))
//      .withColumnRenamed("_c1", sf.SID)
//      .withColumnRenamed("_c6", sf.ROLE_NAME)
//      .withColumn(sf.RID, col(sf.ID))
//      .withColumnRenamed("_c8", sf.LOG_DATE)
//      .withColumn(sf.GAME_CODE,lit("ygh5"))
//      .withColumn(sf.ACTION,action(col("_c5")))
//    join=join.where("log_date like '"+logDate+"%' and sid != '10000'")
//    join=join.select(sf.ID,sf.RID,sf.SID,sf.LOG_DATE,sf.ACTION,sf.GAME_CODE,sf.ROLE_NAME)
//
//
//    registerDs = registerDs.withColumnRenamed("_c3", sf.ID)
...
//    registerDs=registerDs.where("log_date like '"+logDate+"%' or _c27 like '"+logDate+"%'")
//    registerDs=registerDs.select(sf.ID,sf.RID,sf.SID,sf.LOG_DATE,sf.ACTION,sf.GAME_CODE,sf.ROLE_NAME)
//
//    val df = join.union(registerDs)
//    df
//  }
  ...
  override def getActivityDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      ...
      val patternlLoginRegister = Constants.GAMELOG_DIR + "/ygh5/[yyyyMMdd]/Log/daily/*/roletb_user_bk_[yyyyMMdd].csv"
      val registerPath = PathUtils.generateLogPathDaily(patternlLoginRegister, logDate,1)
  ...
  override def getPaymentDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == "") {
      val pattern = Constants.GAMELOG_DIR + "/ygh5/[yyyyMMdd]/Log/daily/money_tb_iap_[yyyyMMdd].csv"
    ...
    registerDs=registerDs.select("_c3","_c0","_c2").where("_c2 != '10000'")
