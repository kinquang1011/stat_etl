...
/**
  ...
  */
class Jx1f extends DbgGameFormatter("jx1f"){
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val patternNewReg = Constants.GAMELOG_DIR + "/gslog/jxf/[yyyy-MM-dd]/new_register.*.csv"
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      val patternPathLogin = Constants.GAMELOG_DIR + "/gslog/jxf/[yyyy-MM-dd]/*/login_logout.*.csv"
    ...
    val filterlog = (line:Array[String]) => {
      var rs = false
      if (line(0).startsWith(logDate) && !line(1).isEmpty && line.length>=8){
        rs = true
      }
      rs
    }
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line)).map { line =>
      ...
      val action = "login"
      val serverId = line(8)
      ("jx1f", dateTime,id,serverId,action,roleId)
    }
