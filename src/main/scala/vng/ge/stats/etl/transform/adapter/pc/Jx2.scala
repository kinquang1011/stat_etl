...
/**
  ...
  */
class Jx2 extends DbgGameFormatter("jx2"){
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val patternNewReg = Constants.GAMELOG_DIR + "/gslog/jx2/[yyyy-MM-dd]/new_register*.csv"
    ...
    val patternBaseAccount = Constants.GAMELOG_DIR + "/gslog/jx2/[yyyy-MM-dd]/account_base.csv"
    val pathBasAccount = PathUtils.generateLogPathDaily(patternBaseAccount, logDate, 1)
    val rawBaseAccount = getRawLog(pathBasAccount)

    val baseDf = rawBaseAccount.map(line => line.split("\\t")).map { line =>
      val  accountName= line(0)
      val id = line(1)
      (accountName,id)
    }.toDF("accountName","id")
        var joinDF = newRegDf.as("a").join(baseDf.as("b"), newRegDf("id") === baseDf("id"), "left_outer")
        joinDF = joinDF.select("a.game_code", "a.log_date","b.accountName")
    joinDF=  joinDF.toDF(sf.GAME_CODE,sf.LOG_DATE,sf.ID)
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      val patternPathLogin = Constants.GAMELOG_DIR + "/gslog/jx2/[yyyy-MM-dd]/login_logout*.csv"
    ...
    val filterlog = (line:Array[String]) => {
      var rs = false
      val dbresult = line(10)
      val nresult = line(11)
      val accountName = line(4)
      if (line(1).startsWith(logDate) && dbresult=="1" && nresult=="1" && !accountName.isEmpty){
        rs = true
      }
      rs
    }
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line)).map { line =>
      ...
      val action = "login"
      val serverId = line(3)
      ("jx2", dateTime,id,serverId,action)
    }
