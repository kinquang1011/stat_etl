...
/**
  ...
  */
class Myplay_Tienlen extends  MyplayFormatter ("myplay_tienlen"){
  ...
  override def getPaymentDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == ""){
      val paymentPatternPath = Constants.GAMELOG_DIR + "/myplay_payment_db/[yyyyMMdd]/Cash_tienlen*"
    ...
    val paymentDs = paymentRaw.map(line => line.split("\\t")).filter(line => filterLog(line(6))).map { line =>
      ...
      val money = line(4)
      val id = line(2)
      ("myplay_tienlen", dateTime,id,money, money)
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      val loginPattern = Constants.WAREHOUSE_DIR + "/myplay_tienlen/login/[yyyy-MM-dd]/*"
      val loginPath = PathUtils.generateLogPathDaily(loginPattern, logDate)
      loginRaw = getRawLog(loginPath)
      val logoutPattern = Constants.WAREHOUSE_DIR + "/myplay_tienlen/logout/[yyyy-MM-dd]/*"
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line(0))).map { line =>
      ...
      val ip = line(14)
      val action = "login"
      ("myplay_tienlen", dateTime, action,channel ,id, platform,ip)
    }
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val registerPattern = Constants.WAREHOUSE_DIR + "/myplay_tienlen/new_register/[yyyy-MM-dd]/*"
