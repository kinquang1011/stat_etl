...
/**
  ...
  */
class Myplay_Bida extends MyplayFormatter ("myplay_bida"){
  ...
  override def getPaymentDs(logDate: String, hourly: String): DataFrame = {
    ...
    if (hourly == ""){
      val paymentPatternPath = Constants.GAMELOG_DIR + "/myplay_payment_db/[yyyyMMdd]/Cash_bida9bi*"
    ...
    val paymentDs = paymentRaw.map(line => line.split("\\t")).filter(line => filterLog(line(6))).map { line =>
      ...
      ("myplay_bida", dateTime,id,gross_amt, net_amt)
  ...
  override def getActivityDs(logDate: String, hourly: String): DataFrame = {
    ...
    if(hourly == ""){
      val logPatternPath = Constants.WAREHOUSE_DIR + "/myplay_bida/user/[yyyy-MM-dd]/*"
