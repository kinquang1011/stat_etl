...
/**
  ...
  */
class Zs extends DbgGameFormatter("zs"){
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val patternNewReg = Constants.GAMELOG_DIR + "/gslog/zs/[yyyy-MM-dd]/character_info_new/ZS_basetab_*.csv"
    ...
    val newRegFilter = (line:Array[String]) => {
      var rs = false
      if (line.length>=17 && line(5).startsWith(logDate)) {
        rs = true
      }
      rs
    }
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      val patternPathLogin = Constants.GAMELOG_DIR + "/gslog/zs/[yyyy-MM-dd]/character_info_new/ZS_basetab_*.csv"
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line)).map { line =>
      val dateTime = line(16)
      val id = line(1)
      val serverId = "1"
      val action = "login"
      ("zs", dateTime,id,serverId,action)
    }
