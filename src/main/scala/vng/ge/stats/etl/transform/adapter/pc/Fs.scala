...
/**
  ...
  */
class Fs extends DbgGameFormatter("fs"){
  ...
  override def getIdRegisterDs(logDate: String, _activityDs: DataFrame, _totalAccLoginDs: DataFrame): DataFrame = {
    ...
    val patternNewReg = Constants.GAMELOG_DIR + "/gslog/fs/[yyyy-MM-dd]/account_register.*.csv"
  ...
  override def getActivityDs(logDate: String, hourly:String): DataFrame = {
    ...
    if (hourly == "") {
      ...
      val patternPathLogin = Constants.GAMELOG_DIR + "/gslog/fs/[yyyy-MM-dd]/login_logout/paysys.log"
    ...
    val filterlog = (line:Array[String]) => {
      ...
      if(line.length>=10){
        if (line(1).startsWith(logDate) && line(9)=="1"){
          rs = true
        }
      }
      rs
    ...
    val loginDs = loginRaw.map(line => line.split("\\t")).filter(line => filterlog(line)).map { line =>
      ...
      val serverId = line(3)
      val action = "login"
      ("fs", dateTime,id,rid,serverId,action)
    }
...
//      var rs = false
//      //      if(line.length>=10){
//      //        if (line(1).startsWith(logDate)&&(line(9).isEmpty || line(9) =="1")){
//      //          rs = true
//      //        }
//      //      }else if(line(1).startsWith(logDate)){
//      //        rs = true
//      //
//      //      }
//      if(line.length>=10){
...
//      val serverId = line(3)
//      val action = "login"
//      ("fs", dateTime,id,id,serverId,action)
//    }
//      .toDF(sf.GAME_CODE, sf.LOG_DATE, sf.ID,sf.RID,sf.SID,sf.ACTION)
//
//    loginDs
//  }
  ...
  override def getCcuDs(logDate: String, hourly: String): DataFrame = {
    import sparkSession.implicits._
    val patternPath = Constants.GAMELOG_DIR + "/gslog/fs/[yyyy-MM-dd]/ccu.*.csv"
